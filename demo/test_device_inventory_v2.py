...
SESSION_KEY = auth.getSessionKey(SPLUNK_USERNAME, SPLUNK_PASSWORD)
...
def assert_device_details(expected_device_list, actual_device_list):
    ...
    expected_device_list_updated.sort()
    actual_device_list_updated.sort()
...
def test_merging_devices():
    ...
    with DeviceManager(SESSION_KEY, logger, DEVICE_INVENTORY_LOOKUP_COLLECTION, HOSTNAME_POSTFIXES) as dm:
        ...
        assert merge_messages[0] == "Device(uuid={}) will going to be merged with Device(uuid={}).".format(device_2_uuid, device_1_uuid), "Device(uuid={}) should be merged to Device(uuid={}), but it isn't. merge_messages={}".format(
            device_2_uuid, device_1_uuid, merge_messages
        )
...
def create_kvstore_collection():
    _ = rest.simpleRequest(
        "/servicesNS/nobody/{}/storage/collections/config?output_mode=json".format(APP_NAME),
        method="POST",
        jsonargs=str("name={}".format(DEVICE_INVENTORY_LOOKUP_COLLECTION)),
...
def delete_kvstore_collection():
    _ = rest.simpleRequest(
        "/servicesNS/nobody/{}/storage/collections/config/{}?output_mode=json".format(APP_NAME, DEVICE_INVENTORY_LOOKUP_COLLECTION),
        method="DELETE",
        sessionKey=SESSION_KEY,
        raiseAllErrors=True,
    )
