...
function(TableView, SplunkCommonUtilities, mvc, _){
    ...
    var AllVulnRowExpansionRenderer = TableView.BaseRowExpansionRenderer.extend({
        ...
        render: function($container, rowData) {
            ...
            this._searchManager.executeReusableSearch(`| inputlookup cs_tenable_vuln | search tenable_uuid="${tenable_uuid}" | sort -vul_severity_id, -vul_state
    ...
    var VulTrafficExpansionRenderer2 = TableView.BaseRowExpansionRenderer.extend({
        initialize: function(args) {
            // initialize will run once, so we will set up a search and a chart to be reused.
            this._searchManager = new SplunkCommonUtilities.VSearchManagerUtility();
            this._searchManager.defineReusablePostProcessSearch('vulnerability_port_traffic_search_for_nessus_json', 'vul-traffic-search-manager2');
        ...
        render: function($container, rowData) {
            ...
            //update the search with the HOST_ID that we are interested in
            this._searchManager.executeReusablePostProcessSearch(`| search vul_ip="${vul_ip}"
            | sort Country, - inbound_traffic, - outbound_traffic
            | table src_ip, inbound_traffic, dest_ip, outbound_traffic, Country, Region, City`);
            // $container is the jquery object where we can put out content.
            // In this case we will render our chart and add it to the $container
