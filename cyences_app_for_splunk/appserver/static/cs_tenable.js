...
function(TableView, SearchManager, mvc, _){
    ...
    var AllVulnRowExpansionRenderer = TableView.BaseRowExpansionRenderer.extend({
        ...
        render: function($container, rowData) {
            ...
            let allIPs = _(rowData.cells).find(function (cell) {
               return cell.field === 'tenable_all_ips';
            }).value;
            if(allIPs == "null"){
                allIPs = _(rowData.cells).find(function (cell) {
                    return cell.field === 'ip';
                }).value;
            }
            let host = _(rowData.cells).find(function (cell) {
                return cell.field === 'host';
            }).value;
            //update the search with the HOST_ID that we are interested in
            this._searchManager.set({ search: `| inputlookup cs_tenable_vuln | search ip IN (${allIPs}) host="${host}" | sort -vul_severity_id, -vul_state
